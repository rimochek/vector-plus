  ...
  return (
    <div className="fixed inset-0 z-[300] flex items-center justify-center p-4">
      <button
        ...
        onClick={onClose}
      />
      <div className="relative z-10 flex max-h-[92vh] w-full max-w-5xl flex-col overflow-hidden rounded-[2rem] border border-slate-100 bg-white shadow-2xl dark:border-zinc-800 dark:bg-zinc-950">
        <div className="flex shrink-0 items-start justify-between gap-4 border-b border-slate-100 px-6 py-5 dark:border-zinc-800 sm:px-8">
          <div>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-4 py-5 sm:px-6">
          {success ? (
            <p className="rounded-2xl bg-violet-50 p-8 text-center font-bold text-[#8B5CF6] dark:bg-violet-950/40">
            ...
            <p className="rounded-2xl border border-dashed border-slate-200 p-8 text-center font-semibold text-slate-400 dark:border-zinc-700">
              {t("booking.noSlots")}
            </p>
          ) : (
            <>
              <p className="mb-4 text-sm font-semibold text-slate-400 dark:text-zinc-500">
                            ...
                            return (
                              <button
                                ...
                                className={`rounded-md border px-1 py-2 text-center text-[11px] font-bold transition ${
                                  isSelected
                                    ? "border-[#8B5CF6] bg-[#8B5CF6] text-white shadow-md shadow-violet-200 dark:shadow-violet-950/40"
                                    : "border-slate-200 bg-white text-[#2563EB] hover:border-[#8B5CF6] hover:bg-violet-50 dark:border-zinc-700 dark:bg-zinc-900 dark:text-blue-400 dark:hover:bg-violet-950/30"
                                }`}
                              >
        ...
        {!success && !loading && slots.length > 0 && (
          <div className="shrink-0 space-y-4 border-t border-slate-100 px-6 py-5 dark:border-zinc-800 sm:px-8">
            <label className="block">
              ...
              <textarea
                ...
                className="w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-medium outline-none focus:border-[#8B5CF6] dark:border-zinc-700 dark:bg-zinc-900"
              />
            </label>
            <button
              type="button"
              ...
              onClick={handleBook}
              className="w-full rounded-2xl bg-[#8B5CF6] py-4 text-sm font-black uppercase tracking-widest text-white transition hover:bg-violet-600 disabled:opacity-50"
            >
